...
import { off, child, onValue, set, update } from "firebase/database";
// Google STUN servers
 ...
 */
interface RTCFireSessionOptions {
  /**
...
export class RTCFireSession {
  ...
  private onAddParticipant(pid) {
    ...
    onValue(readRef, info.onReadValueChange);

    // !!!
    // onDisconnect(writeRef,writeRefOnDisconnect)
  ...
  private onRemoveParticipant(pid) {
    ...
    let { onReadValueChange, readRef, writeRef, writeRefOnDisconnect } =
      this.participantInfo[pid];
    off(readRef, onReadValueChange);
    set(writeRef, null);
  ...
  private processNewIceCandidates(pid) {
    let info = this.participantInfo[pid];
    conn = conn === undefined ? (conn = null) : conn;
  ...
  private async beginNegotiation(pid) {
    ...
    update(writeRef, { offer: sessionDescription });
  }
    ...
    switch (negotiationState) {
      ...
      case NegotiationState.AnswererWaitingForOffer: {
        ...
        if (offer) {
          ...
          update(writeRef, { answer: sessionDescription });
        }
        break;
      }
